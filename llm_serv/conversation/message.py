...
class Message(BaseModel):
    ...
    def validate_content_exists(self) -> "Message":
        if not any(
            ...
            raise ValueError("Message must contain at least one of: non-empty text, image, or document")
        return self
...
if __name__ == "__main__":
    ...
    try:
        ...
        msg = Message(text="Test message with all content types", images=[img], documents=[doc], role=Role.ASSISTANT)
        ...
        if msg.images and loaded_msg.images:
            print(
                f"Image dimensions match: {msg.images[0].width}x{msg.images[0].height} vs {loaded_msg.images[0].width}x{loaded_msg.images[0].height}"
            )

        if msg.documents and loaded_msg.documents:
            print(
                f"Document content length matches: {len(msg.documents[0].content)} vs {len(loaded_msg.documents[0].content)}"
            )
...
"""
    try:
        ...
        doc = Document(content=content, name="test", extension="txt", encoding="utf-8")
    ...
    # Test 2: Image only
    try:
        img = Image(image=None, name="test_image", format="jpg")  # This should fail
    ...
    try:
        doc = Document(content="Test content", name="test_doc", extension="txt")
        msg = Message(documents=[doc])
    ...
    # Test 4: Combination
    try:
        msg = Message(text="Message with everything", images=[img], documents=[doc], role=Role.ASSISTANT)
